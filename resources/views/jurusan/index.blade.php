               ...
               <div class="col" id=card-{{ $jurusan->id }}>
                    <div class="card h-100">
                         @auth
                              <div class="btn-group btn-action">
                                   ...
                                   <form action="{{ route('jurusans.destroy',['jurusan' => $jurusan->id]) }}" method="POST">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="btn btn-danger shadow-none btn-hapus" title="Hapus Jurusan" data-name="{{ $jurusan->nama }}" data-table="jurusan"><i class="fa fa-trash fa-fw"></i></button>
                                   </form>
                              </div>
                         @endauth
                         <div class="card-body text-center">
                              ...
                              <a href="{{ route('jurusan-dosen',['jurusan_id' => $jurusan->id]) }}" class="btn btn-info d-block">Dosen</a>
                              <a href="{{ route('jurusan-mahasiswa',['jurusan_id' => $jurusan->id]) }}" class="btn btn-success d-block mt-2">Mahasiswa</a>
                              <a href="{{ route('jurusan-matakuliah',['jurusan_id' => $jurusan->id]) }}" class="btn btn-success d-block mt-2">Matakuliah</a>
                         </div>
                    </div>
               </div>
          @endforeach
